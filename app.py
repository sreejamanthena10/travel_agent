...
# --- 3. DUAL-THEME JAVASCRIPT FORCE-OVERRIDE ENGINE ---
# Hardcodes both light and dark styles into absolute structural target tags to stop system overrides
LIGHT_BG = "radial-gradient(circle at 50% 50%, #fee2e2 0%, #fae8ff 35%, #f5f3ff 65%, #e0f2fe 100%)"
DARK_BG = "radial-gradient(circle at 50% 50%, #1e1b4b 0%, #111827 100%)"

col_space, col_toggle = st.columns([8, 2])
with col_toggle:
    is_dark = st.toggle("🌙 Dark Mode Setup", value=False)
...
if is_dark:
    ...
    <style>
        ...
        .card-4 {{ background-color: #1f2937 !important; border: 1px solid #374151 !important; }}
        .card-4 .card-title {{ color: #ffffff !important; }}
        .card-4 .card-desc {{ color: #94a3b8 !important; }}
        .ui-card {{ border-radius: 16px !important; padding: 1.8rem !important; min-height: 220px !important; display: flex !important; flex-direction: column !important; justify-content: space-between !important; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.2) !important; }}
        ...
        [data-testid="stChatMessage"] {{ background-color: rgba(31, 41, 55, 0.85) !important; border: 1px solid #374151 !important; border-radius: 12px !important; }}
    </style>
    """
else:
    CSS_SHEET = f"""
    <style>
        ...
        .card-4 .card-title {{ color: #1e293b !important; }}
        .card-4 .card-desc {{ color: #64748b !important; }}
        .ui-card {{ border-radius: 16px !important; padding: 1.8rem !important; min-height: 220px !important; display: flex !important; flex-direction: column !important; justify-content: space-between !important; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05) !important; }}
        .ui-card .card-title {{ font-size: 1.5rem; font-weight: 700; color: #1e293b !important; margin-bottom: 0.8rem; }}
        .ui-card .card-desc {{ font-size: 0.95rem; color: #64748b !important; line-height: 1.5; }}
        .ui-card .card-icon {{ font-size: 2.2rem; text-align: right; margin-top: auto; }}
        [data-testid="stChatMessage"] {{ background-color: rgba(255, 255, 255, 0.75) !important; border: 1px solid #e2e8f0 !important; border-radius: 12px !important; }}
    </style>
    """
...
# --- 6. GLOBAL DATA FALLBACK SEARCH CONNECTOR ---
